								...
								<div class="row">
									<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
										<div class="all-form-element-inner">
											<!-- 해당 lecture 조회 -->
											<form
												action="${pageContext.request.contextPath}/loginCheck/lectureListByTeacher"
												method="post">
						...
						<h4>
							...
							<c:if test="${sessionLevel == 2 }">
								<div class="btn-custom-groups-one btn-mg-b-10"
									style="float: right;">
									<button type="button" class="btn btn-custon-rounded-four btn-danger" onclick="location.href = '${pageContext.request.contextPath}/loginCheck/addExam' "> <b>+</b> 시험출제 </button>
								</div>
							</c:if>
						</h4>
						<!-- 문제 출제 하기 -->
								...
								<c:if test="${not empty examListByLecture }">
									<c:forEach var="map" items="${examListByLecture}">
										<tr>
											...
											<td><a
												href="${pageContext.request.contextPath}/loginCheck/examOne?examNo=${map.examNo}">
													<button
														class="btn btn-sm btn-custon-rounded-three btn-primary">
														<i class="fa fa-pencil-square-o" aria-hidden="true"></i>시험지
													</button>
											</a></td>
										</tr>
									</c:forEach>
